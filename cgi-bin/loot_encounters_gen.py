...
def getOneRandomMonster(monsterDB, cr, types):
    ...
    if types is None:
        ...
        for act_type in types:
            type_sql += "\'" + act_type + "\',"
        type_sql = type_sql[:-1] + ")"
        cursor.execute('SELECT ' + monster_columns_sql + ' FROM monsters WHERE creature_type in ' + type_sql + ' ORDER BY random() LIMIT 1;')
        randomMonster = cursor.fetchone()
...
def gen_encounter(*args):
    ...
    encounter_design = int(nprandom.choice([-1, 0, 1, 2, 3], p=[0.28, 0.58, 0.08, 0.05, 0.01]))
